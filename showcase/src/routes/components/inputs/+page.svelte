...
<div class="showcase-page">
  ...
  <div>
    <ShowcaseSection id="inputtext" title="InputText">
      <h3>Labeled text input — inline grid layout</h3>
      <div class="preview">
        <InputText id="demo-text" label="Font family" bind:value={textVal} />
...
/>`}</code></pre>
    ...
    <ShowcaseSection id="inputnumber" title="InputNumber">
      <h3>Labeled number input — with and without label</h3>
      <div class="preview">
        <InputNumber id="demo-num" label="Step count" bind:value={numVal} />
...
/>`}</code></pre>
    ...
    <ShowcaseSection id="inputnumberstr" title="InputNumberStr">
      <h3>Number input that binds to a string (for formula entry)</h3>
      <div class="preview">
        <InputNumberStr id="demo-numstr" label="Rate constant" bind:value={numStrVal} />
        <p class="demo-label">Value: "{numStrVal}" (string)</p>
      </div>
      <pre><code>{`<InputNumberStr
  id="rate-constant"
  label="Rate constant"
...
/>`}</code></pre>
    ...
    <ShowcaseSection id="inputnumberoptional" title="InputNumberOptional">
      <h3>Number input with an enable/disable checkbox — for optional parameters</h3>
      <div class="preview">
        <InputNumberOptional
          id="demo-opt"
          valueLabel="Max time"
...
/>`}</code></pre>
    ...
    <ShowcaseSection id="inputcheckbox" title="InputCheckbox">
      <h3>Labeled checkbox — solid or none border</h3>
      <div class="preview">
        <InputCheckbox id="demo-check" label="Enable logging" bind:checked={checkVal} />
        <InputCheckbox id="demo-check2" label="Show grid" bind:checked={checkVal} border="transparent" />
        <p class="demo-label">Checked: {checkVal}</p>
      </div>
      <pre><code>{`<InputCheckbox
  id="enable-log"
  label="Enable logging"
  bind:checked={myBool}
/>
<InputCheckbox ... border="transparent" />`}</code></pre>
    </ShowcaseSection>

    <ShowcaseSection id="inputchoice" title="InputChoice">
      <h3>Labeled select dropdown</h3>
      <div class="preview">
        <InputChoice id="demo-choice" label="Integrator" bind:value={choiceVal}>
      ...
      <pre><code>{`<InputChoice id="integrator" label="Integrator" bind:value={method}>
  <option value="euler">Euler</option>
  <option value="rk45">RK45</option>
  <option value="tsit5">Tsit5</option>
</InputChoice>`}</code></pre>
    </ShowcaseSection>

    <ShowcaseSection id="comparecheckbox" title="CompareCheckbox">
      ...
      <pre><code>{`<CompareCheckbox bind:checked={showComparison} />
<CompareCheckbox
