...
<div class="showcase-page">
  ...
  <div>
    <ShowcaseSection id="hero" title="Hero">
      ...
      <pre><code>{`<Hero src={heroImage}>
  <h1>The Matuszyńska Lab</h1>
  <p>Computational Biology · RWTH Aachen</p>
</Hero>`}</code></pre>
    </ShowcaseSection>

    <ShowcaseSection id="herogradient" title="HeroGradient">
      ...
      <div class="preview note">
        <Text color="secondary">
          ...
          primary-color gradient overlay and an overlaid logo image.
          It requires <code>src</code> (background image) and
          <code>cpblLogo</code> (logo image) props — both site-provided image
          URLs or imports.
        </Text>
      </div>
      <pre><code>{`import heroImg from "$lib/assets/hero.jpg";
import cpblLogo from "$lib/assets/cpbl-logo.svg";

<HeroGradient src={heroImg} cpblLogo={cpblLogo}>
  <h1>The Matuszyńska Lab</h1>
  <p>Computational Biology · RWTH Aachen</p>
</HeroGradient>`}</code></pre>
    </ShowcaseSection>

    <ShowcaseSection id="section" title="Section">
      <h3>Content section — constrained to max-width with responsive padding</h3>
      {#each sectionVariants as variant (variant)}
        <div class="preview" style="padding: 0; overflow: hidden; margin-bottom: var(--space-3)">
          <Section {variant}>
            <div class="section-demo">
              <strong>variant="{variant}"</strong>
              <p>Content constrained to <code>--max-width</code> with responsive padding.</p>
            </div>
          </Section>
        </div>
      {/each}
      <pre><code>{`<Section variant="light">…</Section>
<Section variant="surface">…</Section>
<Section variant="primary">…</Section>
<Section variant="accent">…</Section>`}</code></pre>
    </ShowcaseSection>

    <ShowcaseSection id="sectionheader" title="SectionHeader">
      <h3>Primary-colored section header — full-width, min 12rem tall</h3>
      <div class="preview" style="padding: 0; overflow: hidden">
        <SectionHeader>
          <h1 style="color: var(--color-text-inverse); margin: 0;">Research</h1>
          <p style="color: var(--color-text-inverse); opacity: 0.8; margin: var(--space-2) 0 0">
  ...
  <Text color="light">Computational photosynthesis and beyond</Text>
    ...
    <ShowcaseSection id="sectionheaderperson" title="SectionHeaderPerson">
...
/>`}</code></pre>
    ...
    <ShowcaseSection id="boxheadingmain" title="BoxHeadingMain">
      <h3>Large styled heading box — adapts to screen size</h3>
      <div class="preview">
        <BoxHeadingMain n="01" title="Research" color="dark" />
      </div>
      <div class="preview-primary">
        <BoxHeadingMain n="02" title="Publications" color="light" />
      </div>
      <pre><code>{`<BoxHeadingMain n="01" title="Research" />
<BoxHeadingMain n="02" title="Publications" color="light" />
<BoxHeadingMain n="03" title="Team" color="primary" />`}</code></pre>
    </ShowcaseSection>
